...
//TODO: reinstate Attrib type (better OT semantics)
...
// returns [ops, selection] or null
Node.prototype.execCommand = function(command, sel, args) {
	var ops = new Operations();
	var start = sel.start();
	var end = sel.end();
	var nsel = this._execCommand(ops, command, start, end, args);
	if (ops.ops.length > 1) {
		// ie more than just retain
		ops.end(this.length);
		return [ops, nsel]; 
		//nsel might be blank and then the store decides
	}
	return null;
};
//TODO: Table needs a special execCommand that works at the cell level
Node.prototype._execCommand = function(ops, command, start, end, args) {
	if (typeof this[command] === "function") {
		return this[command](ops, start, end, args);
	}
	ops.retain(1); //skip prefix node
	var off = 1;
	var sel;
	for (var i = 0; i < this.children.length && off < end; i++) {
		var child = this.children[i];
		var cl = child.length || 1;
		if (off + cl > start) {
			nsel = child._execCommand(
				ops, command, start - off, end - off, args);
			if (!sel) sel = nsel;
		}
		else ops.retain(cl);
		off += cl;
	};
	ops.retain(this.length - off);
	return sel;
};
Node.prototype.toJSON = function() {
	...
	for (var i = 0; i < this.children.length; i++) {
		...
		ret.children.push(c.toJSON ? c.toJSON() : c);
	}
...
function fromJSON(d) {
	...
	if (d.tag)    return new Tag(d.tag, d.value);
	if (d.endTag) return new EndTag(d.endTag);
	if (!KLASS.hasOwnProperty(d.type)) return d;
	if (d.children)
		children = d.children.map(fromJSON);
	return new KLASS[d.type](d.id, children, d.attributes);
}

function apply(doc, op1) {
...
function transformCursor(cursor, op, isOwnOp) {
	if (isOwnOp) return cursor; //TODO
	return cursor.transform(Operations.fromOp(op));
}
